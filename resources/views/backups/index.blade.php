...
<div class="flex min-h-screen bg-gray-100">
    ...
    <main class="flex-1 p-6 lg:p-8 min-w-0">
        <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
            ...
            <form method="POST" action="{{ route('backups.store') }}">@csrf<button class="bg-[#1E2761] text-white rounded-lg px-4 py-2 text-sm font-semibold"><i class="fa-solid fa-plus mr-2"></i>Créer une sauvegarde</button></form>
        </div>
        ...
        @unless($sqlite)<div class="mb-5 rounded-lg bg-amber-50 p-4 text-sm">PostgreSQL : archive chiffrée de la base et des documents distants. La restauration nécessite une procédure contrôlée sur un environnement isolé.</div>@endunless
        <div class="grid md:grid-cols-3 gap-4 mb-6">
            <div class="bg-white border rounded-xl p-4"><p class="text-xs text-gray-400">Planification</p><p class="font-semibold text-[#1E2761] mt-1">02:00 — ordonnanceur externe requis</p></div>
            ...
            <div class="bg-white border rounded-xl p-4"><p class="text-xs text-gray-400">Protection</p><p class="font-semibold text-[#1E2761] mt-1">Chiffrement AES-256 + SHA-256</p></div>
        </div>
        <div class="space-y-3">
