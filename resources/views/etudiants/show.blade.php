...
<main class="flex-1 p-6">
    ...
    <section class="bg-white rounded-xl shadow p-6 mt-4">
        <div class="flex flex-wrap justify-between gap-4"><div><h1 class="text-lg font-bold text-[#1E2761]">{{ $etudiant->prenom }} {{ $etudiant->nom }}</h1><x-statut-badge :statut="$etudiant->statut_etudiant" /><p class="text-sm text-gray-500 mt-2">{{ $etudiant->email }} · {{ $etudiant->telephone ?? '—' }}</p></div><div class="flex flex-wrap gap-2"><a href="{{ route('etudiants.documents.index',$etudiant) }}" class="bg-blue-50 text-blue-800 px-4 py-2 rounded-lg">Dossier documentaire</a><a href="{{ route('etudiants.edit',$etudiant) }}" class="bg-gray-100 px-4 py-2 rounded-lg">Modifier l’identité</a><a href="{{ route('etudiants.inscriptions.create',$etudiant) }}" class="bg-amber-500 text-white px-4 py-2 rounded-lg">Nouvelle inscription</a></div></div>
    </section>
