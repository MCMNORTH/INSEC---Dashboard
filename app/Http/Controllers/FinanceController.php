...
class FinanceController extends Controller
{
    ...
    public function facture(Etudiant $etudiant)
    {
        $inscription = $etudiant->inscriptions()->with(['formation', 'anneeAcademique', 'versements'])->latest()->first();

        if (!$inscription) {
            return back()->with('error', "Cet étudiant n'a pas encore d'inscription.");
        }

        return view('finances.facture', compact('etudiant', 'inscription'));
    }

    public function updateVersement(Request $request, Versement $versement)
    {
        $validated = $request->validate([
            'montant' => 'required|integer|min:1',
            'date_versement' => 'required|date',
        ...
        $etudiantId = $versement->inscription->id_etudiant;

        return redirect()->route('finances.index', ['etudiant' => $etudiantId])->with('status', 'Versement modifié.');
    }

    public function destroyVersement(Versement $versement)
    {
        $etudiantId = $versement->inscription->id_etudiant;

        $versement->delete();

        return redirect()->route('finances.index', ['etudiant' => $etudiantId])->with('status', 'Versement supprimé.');
    }
}